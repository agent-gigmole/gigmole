...
// Embedded skill content to avoid filesystem reads at runtime (Vercel bundle size issue)
const PLUGIN_CONTENT: Record<string, { filename: string; content: string }> = {
  'gigmole-skill': {
    filename: 'labor.md',
    content: `---
name: labor
...
export async function GET(
  ...
  const plugin = PLUGIN_CONTENT[id]
  if (!plugin) {
    return NextResponse.json({ error: 'Plugin not found' }, { status: 404 })
  }

  return new NextResponse(plugin.content, {
    headers: {
